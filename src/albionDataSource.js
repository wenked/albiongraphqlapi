...
class AlbionApiDataSource extends apollo_datasource_rest_1.RESTDataSource {
    ...
    getBattles(guildname) {
        return __awaiter(this, void 0, void 0, function* () {
            const data = yield this.get(`search?q=${guildname}`);
            const battles = yield this.get(`battles?offset=0&limit=50&sort=recent&guildId=${data.guilds[0].Id}`);
    ...
    getBattleById(id) {
        return __awaiter(this, void 0, void 0, function* () {
            let offset = 0;
            let events = [];
            const killboard = yield this.get(`battles/${id}`);
            let data = yield this.get(`events/battle/${id}?offset=${offset}&limit=51`);
            for (offset = 0; data.length > 0; offset += 50) {
                data = yield this.get(`events/battle/${id}?offset=${offset}&limit=51`);
                events.push(data);
            }
            ...
            const mergeWithItems = mergePlayers.map((player2) => {
                ...
                if (player2.killFame > 0) {
                    ...
                    return Object.assign(Object.assign({}, player2), { weapon: newplayer !== undefined && newplayer.Equipment.MainHand.Type, role: newplayer !== undefined &&
                ...
                return Object.assign(Object.assign({}, player2), { weapon: newplayer !== undefined && newplayer.Equipment.MainHand.Type, role: newplayer !== undefined && zergUtils_1.getRole(newplayer.Equipment.MainHand.Type), averageIp: newplayer !== undefined && newplayer.AverageItemPower
                        ? newplayer.AverageItemPower
                        : null });
            });
            const handlerResult = () => {
